...
4c. **Remaining follow-ons (enhancements, not safety gaps).** Two pieces complete the picture but are not required for failover safety, which 4a+4b already guarantee: (i) **read-committed replicas** — gate replica reads to the commit index so a read never returns an uncommitted-then-superseded write; and (ii) **automated snapshot transfer** — a replica whose log has diverged from a new leader below its applied point, or that has fallen further behind than the op-log buffer retains, currently reports `resync_required` and is re-synced by hand (backup → restore); streaming that snapshot automatically is the remaining mechanism. Sequenced as dedicated projects (see also the per-database lock-sharding finding in docs/PERFORMANCE.md §4b).

> **Scope note.** Slices 1–3 make steady-state replication self-healing (the common production case: a replica lags on a blip and recovers); 4a makes majority-durability observable; 4b makes the failover election sound on true log positions. What remains (4c) is a read-consistency gate and an automated re-sync convenience — a lagging or divergent minority replica is never elected and never corrupts the committed history, but until automated snapshot transfer lands, healing one that is *below* the buffer window or on a divergent tail is a manual restore.
...
- [~] CL10: self-healing replication + consensus (§2.5) — **shipped**: idempotent apply, gap-aware ordered apply, leader-driven catch-up (slices 1–3), the **commit index** (majority-ack, 4a), and the **election on true `(term, index)`** (4b). Remaining follow-ons (not failover-safety gaps): read-committed replicas + automated snapshot transfer for re-sync (4c)
