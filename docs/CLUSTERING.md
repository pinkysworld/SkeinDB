...
4d. **Snapshot re-sync — logical snapshot export shipped; automated apply is the follow-on.** When a replica has fallen further behind than the op-log retains (or diverged), it reports `resync_required` and needs a full state transfer, not an op replay. The read-only primitive for that now exists: **`cluster.replication.snapshot`** exports the node's entire logical state as a sequence of replayable ops (`schema.create_database` / `schema.create_table` — carrying the table's `auto_inc_next` counters — / chunked `data.insert`), tagged with the log position at which it was taken. Applying those ops to a fresh node faithfully reconstructs every database, table, and row (verified by a round-trip test), so an operator can re-sync a replica with it today, and it is the basis the automated flow plugs into. **Remaining:** the *automated* wipe-and-apply on the replica (triggered from `resync_required`) plus the exact **consistency barrier** — the engine write and the `op_seq` assignment are under separate locks, so capturing "snapshot exactly at position N" (so post-resync catch-up neither skips nor duplicate-applies an op) needs a deliberate barrier; and memory-bounded streaming of very large tables (today each table's rows are gathered before chunking).

4e. **Read-committed replicas (enhancement, not a safety gap).** Gate replica reads to the commit index so a read never returns an uncommitted-then-superseded write. This needs the replica to *apply on commit* (buffer received ops, materialize them only once committed) or MVCC snapshot reads, and it changes replica read semantics — a replica shows nothing until a **majority** holds the write, making reads slightly staler in the common case. A deliberate consistency-vs-freshness trade a dedicated project should weigh. Both 4d-remaining and 4e are sequenced as dedicated projects (see also the per-database lock-sharding finding in docs/PERFORMANCE.md §4b).
...
- [~] CL10: self-healing replication + consensus (§2.5) — **shipped**: idempotent apply, gap-aware ordered apply, leader-driven catch-up (1–3), the **commit index** (majority-ack, 4a), the **election on true `(term, index)`** (4b), **commit-index propagation + durability observability** incl. the consensus-RPC guard fix (4c), and the read-only **logical snapshot export** (`cluster.replication.snapshot`, 4d). Remaining follow-ons (not failover-safety gaps): automated wipe-and-apply re-sync + consistency barrier (rest of 4d), read-committed replicas (4e)
