...
- **Commit index.** Every node advertises its log position on each heartbeat, and the primary computes the **commit index** — the highest `(term, seq)` a **majority** of nodes have applied — reported as `commit_term`/`commit_seq` by `cluster.replication.status`. A write at or below the commit index is durable on a quorum, so it survives any single-node loss and any failover. (Wiring the commit index into the election/reads and reconciling divergent logs across a leadership change is the remaining consensus work — see `docs/CLUSTERING.md` §2.5.)
