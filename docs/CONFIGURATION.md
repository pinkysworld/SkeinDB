...
| `SKEINDB_WAL_SYNC_BATCH` | `1` | WAL group commit: `fsync` a database's write-ahead log once every N committed transactions instead of every one. `1` = fsync every commit (strongest durability). A larger value amortizes the fsync-under-lock cost; it can lose at most `N-1` most-recent commits *per database* on a *power-loss* crash (a clean restart still recovers everything), and recovery always yields a consistent committed prefix. The WAL is partitioned per database (`<data_dir>/wal/<db>/`), so the batch counter is tracked independently for each database. |
...
| `SKEINDB_REPLICATION_LOG_BUFFER_CAP` | `4096` | Size of the primary's in-memory replication op-log ring buffer — how many recent writes a lagging replica can catch up from via op-replay before it must instead **snapshot re-sync** (see *Self-healing replication*). Larger values let a replica fall further behind before a full re-sync is needed, at the cost of more memory. |
...
- **Data-safe candidate selection (log matching on true `(term, index)`).** The election prefers the **most up-to-date** replica by its true replication log position `(applied_term, applied_seq)` — a later term always outranks an earlier one, a higher sequence wins within a term, and the `applied_ops` count survives only as a legacy tie-breaker (then heartbeat, then `node_id`). A voter **refuses any candidate whose `(term, seq)` is behind its own** (the Raft up-to-date rule). Since a committed write is on a majority and a winner needs a majority of votes, the elected primary is guaranteed to hold every committed write **by true log position** — so automatic failover cannot lose acknowledged data.
...
- If a replica has fallen further behind than the buffer still retains (or has diverged), `cluster.replication.fetch` returns `resync_required: true` and the replica **re-syncs automatically**: it pulls a full logical snapshot (**`cluster.replication.snapshot`** — the node's entire state as replayable `schema.create_database` / `schema.create_table` [with `auto_inc_next` counters] / chunked `data.insert` ops), wipes its local databases, applies the snapshot, and adopts the snapshot's log position, from which normal catch-up resumes. The snapshot is taken at a log position exactly consistent with the exported state (a `seq_barrier` drains in-flight log-position assignments before capture), so post-resync catch-up neither skips nor duplicate-applies a write. Best-effort: a failed re-sync aborts and retries on the next tick. The in-memory op-log buffer size (which determines how far behind a replica can fall before a re-sync is needed instead of op-replay) is tunable via `SKEINDB_REPLICATION_LOG_BUFFER_CAP` (default 4096).
- **Commit index.** Every node advertises its log position on each heartbeat, and the primary computes the **commit index** — the highest `(term, seq)` a **majority** of nodes have applied. The primary advertises it on its heartbeats and each replica **adopts** it (monotonically, only from the current primary), so `cluster.replication.status` reports `commit_term`/`commit_seq` on **every** node. `cluster.failover.status` additionally reports each node's `applied_term`/`applied_seq` and its **`commit_lag`** — how far behind the committed point that node is — so you can see which replicas are behind on durability. A write at or below the commit index is durable on a quorum, so it survives any single-node loss and any failover.
- **Read-committed reads (opt-in).** A `query.select` with `"read_committed": true` returns only writes a **majority has acknowledged** — it never surfaces an uncommitted write that a failover could still supersede. It works on **any node**: the primary excludes its own not-yet-majority-acknowledged writes, and a replica excludes its uncommitted tail. It is a normal fresh read on a non-clustered node or a node with no uncommitted tail; an explicit `as_of` takes precedence. (Freshness caveat: the boundary is a millisecond timestamp, so during a burst where many writes land in the same millisecond a read-committed view may conservatively exclude a just-committed write until the next — it never includes an uncommitted one.)

Steady-state lag heals on its own; reconciling logs across a **leadership change** remains best-effort until the final consensus slice (§2.5).
