...
- If a replica has fallen further behind than the buffer still retains, `cluster.replication.fetch` returns `resync_required: true` — that replica needs a full state transfer, not an op replay. **`cluster.replication.snapshot`** (read-only) exports the node's entire logical state as a sequence of replayable ops (`schema.create_database` / `schema.create_table` — with `auto_inc_next` counters — / chunked `data.insert`), tagged with the log position it was taken at; applying those ops to a fresh node reconstructs every database, table, and row. (Automating the wipe-and-apply on the replica, with an exact consistency barrier, is a follow-on — see `docs/CLUSTERING.md` §2.5.)
