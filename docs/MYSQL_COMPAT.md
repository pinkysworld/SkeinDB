...
- The listener supports a `COM_QUERY` translation subset through `sql.exec` for:
  - `SELECT` (literal-only, single-table, and simple `INNER JOIN` / `LEFT JOIN` / `RIGHT JOIN` / `CROSS JOIN` queries, including basic left-associative multi-join chains, top-level comma-separated `FROM` lists, baseline `JOIN ... USING (...)` rewrites for simple base-table joins with explicit/qualified projections, projection aliases with or without `AS`, and wildcard projections such as `SELECT *` / `table.*` / `db.table.*` over those supported join shapes)
...
- The MySQL wire layer also ships **compatibility shims** for the checked-in corpus in `tests/compat/corpus.sql`, including:
  ...
  - `DESCRIBE` / `SHOW KEYS`
  - aggregate result emulation for `COUNT(*)`, `COUNT(col)`, `COUNT(DISTINCT col)`, `SUM(col)`, `MIN(col)`, `MAX(col)`, `AVG(col)`, and `GROUP_CONCAT()` on both single-result aggregate queries and simple single-column `GROUP BY` queries (including compatibility-level single-result `HAVING` without `GROUP BY`, grouped `HAVING` for simple alias/aggregate-expression top-level `AND` predicates, and compatibility-level `ORDER BY` / `LIMIT` / `OFFSET` handling)
  ...
  - `information_schema.statistics` virtual table (empty result with correct schema)
