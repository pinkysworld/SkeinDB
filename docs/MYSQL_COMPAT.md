...
2) broaden SQL and function compatibility with stricter parity tests beyond the bundled corpus (deeper correlated/nested subqueries beyond the current recursive `IN` / `EXISTS` / simple scalar-compare compatibility path, negated boolean-tree wrappers, and equality-based correlated rewrite shapes, broader function/date/time/cast parity beyond the current scalar + date/time baseline, and broader `ALTER TABLE` variants beyond the current `ADD/MODIFY/CHANGE/RENAME COLUMN/RENAME TO/DROP COLUMN` plus index metadata surface)
