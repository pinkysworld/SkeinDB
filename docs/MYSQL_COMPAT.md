...
- The listener supports a `COM_QUERY` translation subset through `sql.exec` for:
  - `SELECT` (literal-only, single-table, and simple `INNER JOIN` / `LEFT JOIN` / `RIGHT JOIN` queries, including basic left-associative multi-join chains and baseline `JOIN ... USING (...)` support)
  - `SHOW` (`DATABASES`, `TABLES`, `COLUMNS`)
  - `USE`
  - `CREATE DATABASE`, `CREATE TABLE`, `CREATE [UNIQUE] INDEX`, `DROP INDEX`, `DROP TABLE`
  - `ALTER TABLE ... ADD COLUMN` (including compatibility handling for `AFTER` / `FIRST` position clauses), `ALTER TABLE ... MODIFY [COLUMN]`, `ALTER TABLE ... CHANGE [COLUMN]`, `ALTER TABLE ... RENAME COLUMN`, `ALTER TABLE ... ADD [UNIQUE] KEY`, `ALTER TABLE ... RENAME [KEY|INDEX]`, and `ALTER TABLE ... DROP [KEY|INDEX]`
...
- The MySQL wire layer also now includes a **basic prepared-statement baseline**:
  ...
  - simple prepared `SELECT`s now advertise prepare-time result column counts and MySQL-style column definitions (including single-table `SELECT *`, join wildcard projections, `JOIN ... USING (...)` wildcard coalescing, qualified wildcard projections such as `table.*`, aggregate compatibility queries with basic `HAVING`, and simple join projections)
...
- The MySQL wire layer also ships **compatibility shims** for the checked-in corpus in `tests/compat/corpus.sql`, including:
  ...
  - multi-count WordPress admin aggregates such as `COUNT(NULLIF(<predicate>, false))` alongside `COUNT(*)` for role/user tallies
  ...
  - date-part helpers used by WordPress admin archive filters such as `YEAR(post_date)` and `MONTH(post_date)`
  ...
  - WordPress Site Health-style `information_schema.TABLES` storage-summary queries, including `TABLE_ROWS`, `DATA_LENGTH`, `INDEX_LENGTH`, and grouped `SUM(data_length + index_length) ... GROUP BY TABLE_NAME`
  - limited single-predicate subquery compatibility rewrites for common adoption paths: `... WHERE <col> [NOT] IN (SELECT ...)` and `... WHERE [NOT] EXISTS (SELECT ...)`
...
- A fresh local WordPress smoke test now completes install, login, page/editor flows, and a broad `/wp-admin/` click-through (Posts, Pages, Users, Media, Comments, Settings, Themes, Site Editor, and related admin screens) without database errors against SkeinDB's MySQL listener. Remaining follow-on gaps still surfaced by deeper WordPress/plugin cleanup flows include `REGEXP` filters, multi-table `DELETE ... FROM ...`, taxonomy-related row-shape warnings, and deeper correlated cleanup queries.
...
2) broaden SQL and function compatibility with stricter parity tests beyond the bundled corpus (deeper subqueries, richer aggregate semantics beyond the current basic `HAVING` shim, and broader `ALTER TABLE` variants beyond `ADD COLUMN` / `MODIFY` / `CHANGE` / `RENAME COLUMN` / `ADD|DROP|RENAME [KEY|INDEX]`)
