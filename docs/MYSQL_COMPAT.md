...
- The CLI `--mysql` listener now supports a **MySQL wire handshake** that advertises `caching_sha2_password` (the modern driver default; `mysql_native_password` is disabled in MySQL 8.4 and removed in 9.x). The server also accepts clients that authenticate with `mysql_native_password` directly, and issues an `AuthSwitchRequest` to `mysql_native_password` for any other plugin. Because the server holds the configured token in cleartext it always completes the `caching_sha2_password` *fast-auth* path (`AuthMoreData` fast-auth-success followed by `OK`), so no RSA key exchange or TLS is required for authentication. When no `SKEINDB_TOKEN` is configured the listener runs in trust mode and accepts any client scramble. The `caching_sha2_password` fast-auth path is exercised by genuine clients (the `mysql` CLI, `mysql2`, and PyMySQL) in the real-driver smoke matrix at `tests/smoke/`, gated as the `driver-smoke` CI job.
- **Optional TLS.** Passing `--tls-cert`/`--tls-key` (PEM) to `serve` enables encryption on the MySQL listener: the handshake advertises `CLIENT_SSL`, and a client that sets the flag triggers a rustls TLS upgrade (short `SSLRequest` packet → handshake → encrypted `HandshakeResponse41`) before authentication. Without a configured certificate the handshake does not advertise `CLIENT_SSL` and an `SSLRequest` is answered with an error packet. The same cert/key pair also enables TLS on the PostgreSQL listener.
...
2) deepen prepared-statement parity (complex-query metadata, stricter driver/cursor semantics, fuller protocol coverage) and optimizer parity for production drivers, even though prepare-time metadata now also covers supported scalar-expression projections including baseline arithmetic, broader scalar/date-time functions including `FIND_IN_SET` / `ISNULL`, `DATE_FORMAT` / `FROM_UNIXTIME`, `DATEDIFF` / `TIMESTAMPDIFF`, `WEEKDAY` / `DAYOFWEEK` / `DAYOFYEAR`, `MONTHNAME` / `DAYNAME`, `QUARTER`, `LAST_DAY`, `EXTRACT(<unit> FROM ...)`, and baseline interval arithmetic through `DATE_ADD` / `DATE_SUB` / `TIMESTAMPADD`, supported subquery-compat `SELECT`s whose `WHERE` clauses rewrite cleanly, the current projection-level scalar-subquery slice, embedded scalar-subquery arithmetic, plus `CASE` / `CAST` and simple aggregate / grouped-aggregate compatibility shims
