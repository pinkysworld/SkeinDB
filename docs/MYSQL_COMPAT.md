...
- The MySQL wire layer also ships **compatibility shims** for the checked-in corpus in `tests/compat/corpus.sql`, including:
  ...
  - `information_schema.tables` virtual table with real table metadata, including compatibility `TABLE_ROWS`, `DATA_LENGTH`, and `INDEX_LENGTH` columns plus WordPress Site Health-style grouped `SUM(data_length + index_length)` summaries by `TABLE_NAME`
