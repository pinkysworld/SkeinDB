...
**Recommended sequencing when scheduled:** (i) per-database WAL + sidecar directories with per-DB recovery (independently shippable); (ii) the lock restructure via approach 1 on a feature branch, gated behind a config flag and validated against `bench_multidb_write_throughput` (the pass criterion is that N-database throughput scales up, not down, with N); (iii) cross-database transaction/query coordination and multi-lock ordering; (iv) flip the default once the benchmark and the full suite are green. This is a focused multi-week effort best started at the top of a working block, not appended to unrelated work.

### Snapshot streaming for very large tables (bounded follow-on)

`cluster.replication.snapshot` (used for automated re-sync, §Clustering 2.5) gathers a table's rows into memory before chunking them into `data.insert` ops. For a database whose largest table does not fit comfortably in memory *twice* (the engine's copy plus the export), that is a memory spike on the primary during a re-sync. It is a bounded edge (re-sync is rare, and `SKEINDB_REPLICATION_LOG_BUFFER_CAP` widens how far a replica can fall behind before a re-sync is needed at all). A memory-bounded version needs a cursor-paginated snapshot RPC backed by *indexed* row access (`rows[offset..offset+limit]` without a full re-scan), which is an engine change — deferred as a scale optimization, not a correctness gap.
