...
| `objects.*` | `objects.need`, `objects.missing`, `objects.fetch`, `objects.pull` | Experimental | ValueID object transfer for shard moves and replication. `objects.fetch` preserves the legacy response by default; optional `transfer_only: true` returns only `id` + canonical `entry_b64`, and `objects.pull` requests that compact mode automatically. |
