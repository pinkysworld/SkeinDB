...
| `objects.*` | `objects.need`, `objects.missing`, `objects.fetch`, `objects.pull` plus HTTP `/api/v1/objects/fetch-bin` | Experimental | ValueID object transfer for shard moves and replication. Legacy `objects.fetch` remains unchanged; `transfer_only: true` keeps the compact JSON form. New `objects.pull` prefers the binary fetch endpoint, whose v1 response carries canonical transfer-entry bytes directly, and falls back to compact JSON-RPC on HTTP 404/405 for older sources. |
