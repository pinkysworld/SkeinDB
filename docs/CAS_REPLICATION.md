...
### 7.4) CR07 batch-size sweep and default selection

After CR05 removed duplicate payload representation and CR06 removed per-batch connection churn, the remaining request-level overhead is dominated by HTTP/JSON-RPC framing per fetch batch. CR07 therefore sweeps `objects.pull.batch_size` across 16, 32, 64, 128, and 256 while holding the transfer format, keep-alive behavior, workload, and source object set constant.

The live sweep covers the low-redundancy/high-churn and balanced scenarios, because both contain enough missing objects to exercise multiple batches. Results are aggregated across zero-overlap and CAS-overlap pulls:

| Batch size | Combined HTTP bytes | Change vs 32 | Zero-overlap batches | CAS-overlap batches |
|---:|---:|---:|---:|---:|
| 16 | 150,108 | +6.21% | 25 | 16 |
| 32 | 141,327 | baseline | 13 | 8 |
| **64** | **136,927** | **-3.11%** | **7** | **4** |
| 128 | 135,159 | -4.36% | 4 | 3 |
| 256 | 133,835 | -5.30% | 2 | 2 |

The byte curve has a clear knee at the existing production default of **64**. Relative to 64, moving to 128 saves only about **1.29%** additional combined HTTP bytes; moving to 256 saves about **2.26%**. Those incremental byte reductions come with larger single responses and coarser retry granularity.

CR07 therefore **validates and retains the default batch size of 64** rather than changing runtime behavior. The engine continues to accept explicit caller-selected sizes up to the existing maximum of 512.

The checked-in evidence is `eval/reports/cas_batch_sweep_ci.{json,md}`, generated by `eval/cas_batch_sweep.py` and diff-gated in CI. This is a byte-efficiency conclusion only; no latency or throughput conclusion is drawn from CI wall-clock time.

### 7.6) CR09 binary replication fetch response

CR09 removes the remaining Base64 and repeated response-ID overhead from the replication response while preserving backward compatibility.

New destinations first POST the usual JSON request body to:
...
repeat count times:
  length   u32 LE
  payload  canonical encode_transfer_entry bytes
```

The canonical transfer entry already carries its ValueID, so the binary response does not repeat hexadecimal IDs. If an older source returns HTTP 404 or 405, the destination automatically falls back to the CR05 compact JSON-RPC `objects.fetch` path. The binary endpoint applies the same token/RBAC authorization policy as `objects.fetch`.

The preserved pre-CR09 wire snapshot is `eval/reports/cas_http_wire_pre_cr09.{json,md}`. With the same workload:

| Scenario | Pre-CR09 zero-overlap | CR09 zero-overlap | Reduction | Pre wire/materialized value | CR09 wire/materialized value |
|---|---:|---:|---:|---:|---:|
| low redundancy / high churn | 56,564 B | 34,361 B | 39.25% | 1.00x | 0.61x |
| balanced | 30,280 B | 18,426 B | 39.15% | 1.01x | 0.62x |
| high redundancy / low churn | 6,677 B | 4,059 B | 39.21% | 1.01x | 0.61x |

The CAS-overlap transfers fall from 42,370 -> 25,739 B, 12,113 -> 7,371 B, and 1,267 -> 807 B respectively. The first two are again about 39% lower; the tiny four-object residual transfer improves by 36.31% because fixed request/HTTP costs dominate.

CR09 wire-composition at batch size 64 shows the new residual structure:

| Scenario | Wire bytes | Request IDs | Request envelope+HTTP | Binary transfer entries | Binary framing | Response envelope+HTTP |
|---|---:|---:|---:|---:|---:|---:|
| low redundancy / high churn | 24,806 | 6,688 (27.0%) | 423 | 16,424 (66.2%) | 791 | 480 |
| balanced | 7,061 | 1,891 (26.8%) | 141 | 4,644 (65.8%) | 225 | 160 |

The decoded canonical transfer entries are only about **0.389x** the materialized ValueStore-byte counter because delta transfer entries can be smaller than materialized values. Therefore CR09 wire/materialized-value ratios below 1 are valid and must not be described as "negative protocol overhead."

The next transport target is now the **request side**: roughly 27% of the remaining wire bytes are 32-character hexadecimal ValueIDs embedded in JSON. A future binary request form can replace those with raw 16-byte IDs while keeping the CR09 binary response unchanged.

Evidence:
- `eval/reports/cas_http_wire_ci.{json,md}`
- `eval/reports/cas_http_wire_pre_cr09.{json,md}`
- `eval/reports/wire_composition_ci.{json,md}`
- `eval/reports/wire_composition_pre_cr09.{json,md}`


### 7.5) CR08 wire-composition analysis

CR08 captures and parses the exact HTTP/1.1 TCP payload used by the CR05 compact-transfer, CR06 keep-alive path at the validated batch size of 64. Every measured byte is assigned to a wire category, and the categories must close exactly to the transparent-proxy total.

| Scenario | Total wire | Request IDs | Request envelope + HTTP | `entry_b64` JSON | Response IDs | Response envelope + HTTP |
|---|---:|---:|---:|---:|---:|---:|
| low redundancy / high churn | 41,050 B | 6,688 B (16.3%) | 681 B | 22,534 B (54.9%) | 6,494 B (15.8%) | 4,653 B |
| balanced | 11,673 B | 1,891 B (16.2%) | 227 B | 6,372 B (54.6%) | 1,836 B (15.7%) | 1,347 B |

HTTP headers themselves are only about 2% of the wire total. The dominant remaining costs are therefore:

1. Base64-encoded transfer entries at about **54.6-54.9%** of total wire bytes.
2. Request-side hexadecimal ValueIDs at about **16.2-16.3%**.
3. Response-side repeated ValueIDs at about **15.7-15.8%**.
4. Remaining response JSON-RPC structure at about **10.3%**.

The Base64 text itself is about **1.349x** the decoded transfer-entry size, matching the expected encoding expansion. The decoded canonical transfer entries are only about **38.9%** of the source-side ValueStore object-byte counter in these workloads because the transfer representation preserves SkeinDB's compact/delta structure.

CR08 therefore selects a **binary replication fetch path** as the next protocol optimization. Additional HTTP-header tuning or larger batches cannot address the dominant remaining byte categories. A binary response can carry canonical transfer entries directly, avoiding Base64 and repeated response IDs; a later binary request form can also replace 32-character hexadecimal IDs with raw 16-byte ValueIDs.
