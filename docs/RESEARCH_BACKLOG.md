...
|---|---|---|
| R19 Wasm query operators | `wasm.plan.compile/run/inspect/perf_report/edge_package` exists with generated fixed-width artifacts and host fallback. Integer `div` stays host-interpreted to preserve floating-point SQL semantics; older artifacts with unsupported generated expression shapes are normalized to that fallback. | Production SIMD-lowered codegen and broader hardened operator coverage are not yet claimed. Tests cover generated signed integer comparisons/arithmetic, host fallback, and integer-division result parity. See TRUE_STATUS_MATRIX. |

## Recent verified closures
- **2026-09-24:** R19 integer-division correctness slice: generated Wasm no longer accepts `div` because the host evaluator returns floating-point results while Wasm integer opcodes truncate. Older generated artifacts containing operators outside the verified codegen subset are normalized to the host evaluator, including the edge-package output. Evidence: `engine::tests::wasm_plan_integer_division_uses_host_semantics_for_new_and_old_artifacts`.
...
- **2026-06-11:** R18 micro-slice (pacing wiring + fidelity): `inject_replay_timing` wired into `apply_simulated_pacing` (simple deterministic sim delays in replay run internal exec) + new unit test `engine::tests::replay_pacing_applies_injected_delays_for_replay_exec_fidelity` + updated integration in cluster_rpc::t188 (exercises replay/RPC) + engine tests. Full AGENTS (deep dive, <200LOC net, unit+RPC int tests, fmt/clippy/test, matrix/backlogs/site, hard review, commit ref review/matrix/R18/AGENTS, push). Historical micro-slice note; the Sep 22 hardened-baseline closure above supersedes its scope/status wording.
- **2026-06-11:** R18 micro-slice (timing injection + fidelity): `inject_replay_timing` (R18 primitive for deterministic delay sim from profile) added + exercised in maintenance_replay_run (with LSM stats_snapshot for recon fidelity) + dedicated unit test `engine::tests::replay_timing_injection_simulates_deterministic_pacing` + coverage/asserts in roundtrip + rehydrate tests. Refs in matrix. Full AGENTS followed (fmt/clippy/test relevant, updates, site, review, commit ref review/matrix/R18). Historical micro-slice note; the Sep 22 hardened-baseline closure above supersedes its scope/status wording.
