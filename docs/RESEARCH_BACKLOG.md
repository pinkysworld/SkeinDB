...
- **18 tracks are now hardened** with real algorithms and dedicated tests:
  ...
  - **R15** — Conflict-free schema evolution (propose/merge/apply integration test, MVCC row schema-version tags, concurrent add column/index merges, query-time adaptation for heterogeneous rows, deterministic merge roll-forward/rollback resolution, divergence/resolution migration assistant guidance, rolling-deploy simulation harness, SkeinAdmin schema-evolution page)
...
- Checklist count: **92 done / 17 open** after closing the restricted Wasm filter/project codegen slice. SIMD exploration, standalone in-edge execution, deeper performance replay injection, and geo-routing bundle windows remain open.
...
- 2026-05-16 (R19 restricted Wasm codegen): T085 is closed with `generated_filter_project_v1` artifacts for fixed-width non-null `u64`/`bool` filter/project plans, embedded Wasm module metadata inside `wasm.plan.compile`, Wasmtime-backed execution in `wasm.plan.run`, explicit host fallback for unsupported types, and focused coverage in `engine::tests::wasm_plan_compile_and_run`, `engine::tests::wasm_plan_compile_falls_back_for_unsupported_types`, `server::tests::wasm_plan_compile_run_rpc`, `server::tests::wasm_plan_run_batch_rpc`, and `tests/quic_rpc.rs::quic_wasm_plan_run_batch`. Counts now 92 done / 17 open.
- 2026-05-16 (R15 SkeinAdmin schema evolution): T326 is closed with a dedicated Schema Evolution (R15) page in SkeinAdmin's Schema panel, typed controls for `schema.propose_change`, `schema.merge_status`, `schema.simulate_rollout`, and `schema.apply_merge`, plus focused asset coverage in `skeinadmin_schema_panel_exposes_r15_evolution_controls`. Counts now 91 done / 18 open.
- 2026-05-16 (R15 rolling deploy simulation): T325 is closed with `schema.simulate_rollout`, which projects prepare/mixed/steady-state rollout stages across configurable node counts, carries forward `schema.merge_status` resolution guidance, and reports when existing rows still require query-time schema adaptation after the merged schema version lands. Evidence: `schema_simulate_rollout_reports_mixed_version_waves` and `r15_schema_evolution_concurrent_column_and_index_changes`. Counts now 90 done / 19 open.
- 2026-05-16 (R15 migration assistant): T324 is closed with `schema.merge_status` resolution guidance that classifies pending divergence as `roll_forward`, `rollback`, or `wait`, so callers can see both the raw conflicts and a proposed resolution plan before running `schema.apply_merge`. Evidence: `schema_merge_status_proposes_resolution_actions` and `r15_schema_evolution_concurrent_column_and_index_changes`. Counts now 89 done / 20 open.
- 2026-05-16 (R15 merge resolution): T323 is closed with deterministic merge roll-forward for eligible proposals plus terminal-conflict rollback to `rejected` status during `schema.apply_merge`, so losing proposals no longer remain pending after a merge and RPC callers receive explicit `rolled_back` conflict details. Evidence: `schema_evolution_merges_concurrent_column_and_index_changes` and `r15_schema_evolution_concurrent_column_and_index_changes`. Counts now 88 done / 21 open.
