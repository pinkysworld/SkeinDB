...
- **18 tracks are now hardened** with real algorithms and dedicated tests:
  ...
  - **R15** — Conflict-free schema evolution (propose/merge/apply integration test + MVCC row schema-version tags)
...
- Checklist count: **85 done / 24 open** after closing the R15 MVCC row schema-version tagging slice. Native Wasm query-operator codegen, SIMD, standalone in-edge execution, deeper performance replay injection, and geo-routing bundle windows remain open.
