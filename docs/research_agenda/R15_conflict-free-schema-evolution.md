...
- **SkeinAdmin status (2026-05-16):** SkeinAdmin's Schema panel now exposes a dedicated Schema Evolution (R15) page that sends typed `schema.propose_change`, `schema.merge_status`, `schema.simulate_rollout`, and `schema.apply_merge` payloads so operators can review divergence, rollout stages, and merge outcomes without hand-editing raw RPC JSON. Focused evidence lives in `skeinadmin_assets.rs::skeinadmin_schema_panel_exposes_r15_evolution_controls`.
