...
- [ ] T036: Broaden COM_QUERY parity for WordPress-class workloads (single-column grouped `COUNT`/`SUM` shims, parenthesized `AND`/`OR` filters, and basic left-associative multi-join chains are now covered; subqueries, richer aggregates, and `ALTER TABLE` variants remain)
