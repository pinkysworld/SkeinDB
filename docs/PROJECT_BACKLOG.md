...
- [ ] T036: Broaden COM_QUERY parity for WordPress-class workloads (single-column grouped `COUNT`/`SUM` shims, projection-grouped `GROUP BY` de-dup compatibility for `SQL_CALC_FOUND_ROWS` flows, parenthesized `AND`/`OR` filters plus `NOT IN` / `NOT LIKE`, basic left-associative multi-join chains, `CREATE [UNIQUE] INDEX` / `DROP INDEX`, `ALTER TABLE ... ADD COLUMN` position-clause / `ADD [UNIQUE] KEY` / `DROP [KEY|INDEX]`, bootstrap/session compatibility `SET` forms including `SQL_AUTO_IS_NULL`, literal `SELECT @@...` compatibility for `LIMIT` / `OFFSET` bootstrap probes, compatibility `SHOW VARIABLES` / `SHOW STATUS` values (including unfiltered, scoped, simple `WHERE Variable_name ...`, and wildcard forms such as `character_set_%` / `collation_%`), and `LOCK TABLES` / `UNLOCK TABLES` no-op handling are now covered; subqueries, richer aggregates, and broader `ALTER TABLE` variants remain)
