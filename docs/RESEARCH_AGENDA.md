...
| R15 | Implemented (prototype) | Conflict-aware schema evolution (`schema.propose_change`, `schema.merge_status`, `schema.simulate_rollout`, `schema.apply_merge`, MVCC row schema-version tags, concurrent add column/index merge planning, deterministic merge roll-forward/rollback resolution, divergence/resolution migration-assistant guidance, rolling-deploy simulation harness) |
