...
The formula is `HEAD`-driven until the first tagged release lands. After the tag-driven release workflow updates `Formula/skeindb.rb`, the stable install path is:
...
The repository and signing metadata are generated by `.github/workflows/release-packages.yml`.
