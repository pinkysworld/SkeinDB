...
| Phase 3 MySQL protocol | Implemented (baseline) | Wire listener performs handshake + `mysql_native_password` auth exchange and supports `COM_QUERY` through the SQL-translation subset (`SELECT/SHOW/USE/CREATE DATABASE/CREATE TABLE/DROP TABLE/INSERT/UPDATE/DELETE`, `INSERT ... ON DUPLICATE KEY UPDATE`, and `SQL_CALC_FOUND_ROWS` + `FOUND_ROWS()`), plus wire-level compatibility shims for the bundled MySQL corpus (`VERSION()`, `DATABASE()`, WordPress-style session/bootstrap queries, expanded `SHOW` variants, `DESCRIBE` / `SHOW KEYS`, simple `COUNT(*)`, and the corpus' session-control flow). `tests/compat/corpus.sql` now runs end-to-end over the MySQL listener in integration tests. |
