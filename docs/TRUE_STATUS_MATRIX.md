...
- **Latest verified slice:** PostgreSQL catalog parity now includes `pg_catalog.pg_description` as a correctly typed virtual table alongside `pg_catalog.pg_am`, and PostgreSQL CSV `COPY` now honors single-byte `QUOTE` markers across export and import on the live PG path.
...
| PostgreSQL compatibility | PG listener, startup/auth, many SQL rewrites, catalog probes, COPY text/csv with `NULL` / `HEADER` / `HEADER MATCH` / `DELIMITER` / `QUOTE`, parenthesized and legacy bare `WITH CSV|TEXT|BINARY` format aliases, binary `TO STDOUT`, extended query, SQLSTATE mapping, and corpus coverage exist. | Not full PostgreSQL; binary `COPY FROM STDIN`, broader COPY option coverage beyond the current formats and aliases, wider catalog parity, and driver matrices remain. |
