...
Latest changes: R02/R05/R07/R09/R16 research hardening; window functions (ROW_NUMBER/RANK/DENSE_RANK); SET @var/SELECT @var; BIT_AND/BIT_OR/BIT_XOR aggregates; multi-table UPDATE upgrade; new scalar funcs (DEGREES/RADIANS/PERIOD_ADD/PERIOD_DIFF/MAKEDATE/MAKETIME); Easy Viewer UX polish; admin console feature center + research badges.
...
| Phase 12 SkeinAdmin | Partial | SkeinAdmin is embedded and functional; 22 feature center cards; research dashboard with hardened/prototype status badges; Easy Viewer with row numbers, NULL display, quick-filter, type-aware formatting, inline SQL tab, Ctrl+Enter shortcut, column tooltips, improved insert form with required-field indicators; security/observability hardening remains open. |
