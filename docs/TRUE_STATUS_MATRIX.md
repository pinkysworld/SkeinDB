...
- **2026-06-11:** Micro-changes per hard review (A storage / B monolith / C encryption / D PG/CDC) referencing TRUE_STATUS_MATRIX + reviewer recs: (A) engine.rs core_lsm_files_active now uses core FileHeader::decode for MANIFEST validation + dedicated test; (B) storage_mode.rs gained uses_segment/primary_row_extension + engine refactors + tests (continues monolith split); (C) first EncryptedValueStore usage + test in skeindb crate for secondary routing baseline; (D) server.rs pg_class virtual catalog (empty/typed) + sql_exec test coverage. All follow AGENTS.md (small, tests added, no unsafe, no format bump). Matrix + Phase/partial rows updated with exact locations and evidence. Cargo fmt/clippy/test required on PR.
