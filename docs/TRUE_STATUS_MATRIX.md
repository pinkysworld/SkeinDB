...
| Phase 3 MySQL protocol | Implemented (baseline) | Wire listener performs handshake + `mysql_native_password` auth exchange and supports `COM_QUERY` through the SQL-translation subset (`SELECT/SHOW/USE/CREATE DATABASE/CREATE TABLE/ALTER TABLE ... ADD COLUMN/DROP TABLE/INSERT/UPDATE/DELETE`, `INSERT IGNORE`, `REPLACE`, `INSERT ... ON DUPLICATE KEY UPDATE`, `DISTINCT`, simple `INNER JOIN` / `LEFT JOIN`, and `SQL_CALC_FOUND_ROWS` + `FOUND_ROWS()`). The MySQL layer now also preserves MySQL-style column defaults for `CREATE TABLE` / `ALTER TABLE`, carries `KEY` / `UNIQUE KEY` metadata into `SHOW INDEX` / `SHOW CREATE TABLE`, enforces declared `UNIQUE KEY` semantics via the compatibility layer, and treats `NULL` values in comparison / `IN` / `LIKE` predicates as SQL-style unknowns rather than ordinary values; `REPLACE` and `ON DUPLICATE KEY UPDATE` route duplicate-key handling through declared PK / `UNIQUE KEY` metadata as well (still scan-based rather than index-backed). `tests/compat/corpus.sql` runs end-to-end over the MySQL listener in integration tests, while follow-on backlog items `T035`-`T037` track the remaining parity gaps that still separate this from a full MySQL replacement. |
