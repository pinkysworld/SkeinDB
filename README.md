...
- **Short authority:** [docs/TRUE_STATUS_MATRIX.md](docs/TRUE_STATUS_MATRIX.md) is the current snapshot for compatibility claims, partial areas, and remaining gaps. [docs/PROJECT_BACKLOG.md](docs/PROJECT_BACKLOG.md) and [docs/RESEARCH_BACKLOG.md](docs/RESEARCH_BACKLOG.md) are task inventories.
...
- **WAL-backed row recovery bridge plus streaming large-table scans.** The live row path still serves JSON/segment snapshots, but committed row mutations now bridge through the core WAL primitives, startup replays only newer committed changes, torn WAL tails are truncated, segment-backed large-table scans can stream persisted rows directly, and checkpoint/read-path state is exposed in `stats.snapshot.storage`.
...
- **Vector search** with an HNSW graph index, exact-vs-indexed recall/latency reports, and a credential-free RAG retrieval sample (`vector.insert`, `vector.search`, `vector.benchmark`, `samples/vector_rag_pipeline.py`).
...
- **Incremental materialized views** with persisted definitions, column-granular dependencies, filter/project/group-by maintenance, auto full-refresh fallback, and `view.evaluate` correctness/benchmark reports.
- **Change Data Capture** over polling, SSE, and WebSocket streams, with durable cursors, bounded retention, backpressure/resnapshot signaling, row-image options, primary-key/source-op/changed-column filters, and query invalidations that expand view, set-operation, and CTE dependencies to real base tables.
...
- **PostgreSQL v3** wire baseline with SCRAM-SHA-256 auth, simple + extended query protocol, virtual `pg_catalog` including table/view, role/user, index, tablespace, sequence/statistics, and database-stat probes, transaction/savepoint state, SQLSTATE-mapped errors, and fixture-backed pgAdmin/DBeaver plus psycopg/SQLAlchemy-style introspection coverage.
...
- PostgreSQL support is real but still partial: admin-tool metadata browsing is stronger now, but broader dialect/catalog parity, some COPY/type gaps, and production-grade framework/driver matrices are still open. See [docs/PG_COMPAT.md](docs/PG_COMPAT.md).
...
- **PostgreSQL:** partial PG v3 baseline with trust/SCRAM-SHA-256 auth, managed DB-user passwords, SSL rejection, startup probes, simple + extended query protocol, virtual `pg_catalog` including table/view, role/user, index, tablespace, sequence/statistics, and database-stat probes, SQLSTATE-mapped errors, failed-transaction blocking, and explicit pgAdmin/DBeaver-style introspection fixture coverage.
...
- **CDC:** Phase 23 table/query subscriptions support polling, SSE, WebSocket replay, durable cursors, pause/resume, backpressure, resnapshot signaling, row images, source-op filters, exact primary-key filters, inclusive single-column primary-key ranges, changed-column filters, `objects_json` / `plain_json` / `compact_json` delivery, file sink draining with deterministic cursor handoff, and prepared-query invalidation over direct base tables, view-expanded base tables, set-operation branches, and CTE definitions.
...
- **Storage:** default row persistence is `segment` mode using `.rseg`, with fallback/hybrid support still present and a WAL-backed crash-recovery bridge now protecting committed row mutations between checkpoints.
