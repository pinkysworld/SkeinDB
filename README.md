...
CR09 implements that binary response. New destinations prefer a binary `/api/v1/objects/fetch-bin` response carrying canonical transfer entries directly and fall back automatically to JSON-RPC on older sources. Against the preserved pre-CR09 snapshot, zero-overlap HTTP-over-TCP bytes drop by about **39% again** (56,564->34,361 B and 30,280->18,426 B in the two larger scenarios). The remaining wire composition is now dominated by the canonical transfer bytes plus **~27% request ValueIDs**, making a binary request-ID form the next transport target.
