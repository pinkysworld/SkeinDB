...
- **Opt-in RBAC on the RPC path** (`SKEINDB_RBAC`). Beyond the single shared bearer token, per-role authorization can be enabled so each request resolves to a principal (the `SKEINDB_TOKEN` superuser or an API-token secret → its role) and each method is checked against a `read < write < admin` privilege before dispatch; denied calls return `403`. Off by default (legacy single-token behavior unchanged). See [docs/CONFIGURATION.md](docs/CONFIGURATION.md#rbac-role-based-access-control-on-the-rpc-path).
